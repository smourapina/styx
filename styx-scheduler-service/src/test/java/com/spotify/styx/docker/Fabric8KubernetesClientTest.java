...
public class Fabric8KubernetesClientTest {
  ...
  @Mock Fabric8KubernetesClient.EventHandler<Pod> eventHandler;
  @Mock Watch watch;

  @Captor public ArgumentCaptor<Watcher<Pod>> podWatcherCaptor;
  ...
  public void shouldWatchPods() throws IOException {
    // Start the watcher
    var watcher = sut.podWatcher(eventHandler);
    watcher.start();

    verify(client).pods();
    verify(pods).watch(podWatcherCaptor.capture());

    // Emulate getting events from k8s
    var podWatcher = podWatcherCaptor.getValue();
    podWatcher.eventReceived(ADDED, createdPod);
    podWatcher.eventReceived(MODIFIED, createdPod);

    // Verify that the event handler gets called
    verify(eventHandler).accept(ADDED, createdPod);
    verify(eventHandler).accept(MODIFIED, createdPod);

    // Simulate losing connection
    podWatcher.onClose(new KubernetesClientException("foobar!"));

    // Wait for reconnect
    verify(client, timeout(30_000).times(2)).pods();
    verify(pods, timeout(30_000).times(2)).watch(podWatcherCaptor.capture());

    // Verify that event handling works after reconnect
    podWatcher.eventReceived(DELETED, deletedPod);
    verify(eventHandler).accept(DELETED, deletedPod);
