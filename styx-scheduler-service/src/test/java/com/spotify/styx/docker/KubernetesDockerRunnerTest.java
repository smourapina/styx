...
import com.spotify.styx.docker.Fabric8KubernetesClient.EventHandler;
import com.spotify.styx.docker.Fabric8KubernetesClient.PodWatcher;
...
public class KubernetesDockerRunnerTest {
  ...
  @Captor private ArgumentCaptor<EventHandler<Pod>> eventHandlerCaptor;
  ...
  private String podName = createdPod.getMetadata().getName();
  private Stats stats = mock(Stats.class);

  private KubernetesDockerRunner kdr;
  private EventHandler<Pod> podEventHandler;
  ...
  public void setUp() {
    ...
    when(k8sClient.podWatcher(eventHandlerCaptor.capture())).thenReturn(podWatcher);
    ...
    verify(k8sClient).podWatcher(any());

    podEventHandler = eventHandlerCaptor.getValue();
  ...
  public void shouldNotCleanupRunningPodWithoutRunStateIfTerminatedAfterRefresh() {
    ...
    when(k8sClient.getPod(podName)).thenReturn(Optional.of(refreshedPod));
  ...
  public void shouldPollPodStatusAndEmitEvents() throws Exception {
    ...
    final Pod terminatedPod = new PodBuilder(createdPod)
        ...
        .build();
    when(k8sClient.getPod(podName)).thenReturn(Optional.of(terminatedPod));
  ...
  public void shouldHandlePodWithoutAnnotationsWhenWatching()  {
    ...
    assertThat(Try.run(() -> podEventHandler.accept(Action.MODIFIED, pod)).isSuccess(), is(true));
  }
