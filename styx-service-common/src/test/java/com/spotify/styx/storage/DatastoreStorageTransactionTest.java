...
public class DatastoreStorageTransactionTest {
  ...
  public void shouldRetryOnConflict() throws Exception {
    ...
    var future = executor.submit(() -> storage.runInTransaction(tx -> {
  ...
  public void shouldStoreAndDeleteWorkflow() throws Exception {
    ...
    storage.runInTransaction(tx -> tx.store(workflow));
    assertThat(storage.workflow(workflow.id()), is(Optional.of(workflow)));

    storage.runInTransaction(tx -> tx.deleteWorkflow(workflow.id()));

    var deleted = storage.runInTransaction(tx -> tx.workflow(workflow.id()));
  ...
  @Test
  public void shouldStoreShards() throws Exception {
    storage.runInTransaction(tx -> {
  ...
  public void shouldGetWorkflow() throws Exception {
    ...
    var read = storage.runInTransaction(tx -> tx.workflow(WORKFLOW_ID));
  ...
  public void shouldPatchState() throws Exception {
    ...
    WorkflowState state = WorkflowState.builder()
        ...
        .build();
    storage.runInTransaction(tx -> tx.patchState(WORKFLOW.id(), state));
  ...
  public void shouldReturnAllActiveStateForWFI() throws Exception {
    ...
    var activeStates = storage.runInTransaction(tx -> tx.readActiveState(WORKFLOW_INSTANCE1));
    assertThat(activeStates, is(Optional.of(RUN_STATE)));
  }

  @Test
  public void shouldInsertActiveState() throws Exception {
    storage.runInTransaction(tx -> tx.writeActiveState(WORKFLOW_INSTANCE1, RUN_STATE));
  ...
  public void shouldUpdateActiveState() throws Exception {
    ...
    storage.runInTransaction(tx -> tx.writeActiveState(WORKFLOW_INSTANCE1, runState));
    var newRunState = RunState.create(WORKFLOW_INSTANCE1, NEW, StateData.zero(), TIMESTAMP, 43L);
    storage.runInTransaction(tx -> tx.updateActiveState(WORKFLOW_INSTANCE1, newRunState));
  ...
  public void shouldDeleteActiveState() throws Exception {
    storage.runInTransaction(tx -> tx.writeActiveState(WORKFLOW_INSTANCE, RUN_STATE1));
    storage.runInTransaction(tx -> tx.deleteActiveState(WORKFLOW_INSTANCE));
  ...
  public void shouldStoreAndGetBackfill() throws Exception {
    var backfill = Backfill.newBuilder()
        ...
        .schedule(DAYS)
        .build();
    storage.runInTransaction(tx -> tx.store(backfill));
    var read = storage.runInTransaction(tx -> tx.backfill(backfill.id()));
